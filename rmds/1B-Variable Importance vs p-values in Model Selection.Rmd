...
options(width = 90)  #adjust width of console outout to allow wide tables to fit
...
**Comparing p-values vs Importance Weights**
...
#p-values from global model
p_list
```

`pct.cover`, and `slope` appear in two of the three the top-ranked candidate models (AICc \~ \<2.0) even though they are not 'significant' in the global model (slope p = 0.77, pct.cover p = 0.30)
