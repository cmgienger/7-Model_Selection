...
## Washburn Example (Yellowstone)

Aho and Weaver (2010) examined the effect of environmental characteristics on alpine vascular plant species richness on Mount Washburn (3124 m) a volcanic peak in north-central Yellowstone National Park. The established 40 plots and measured species richness along with a suite of environmental variables; soil nitrogen, slope, aspect, rock cover, and soil pH.

https://www.rdocumentation.org/packages/asbio/versions/1.6-7/topics/wash.rich

![](../images/washburn1.jpg){width=400px} ![](../images/washburn2.jpeg){width=500px} ![](../images/effect_of_topography.png){width=500px}

```{r import and reformat data}
data("wash.rich") #attach data from the asbio library
data_washburn <- rename(wash.rich,
                        site = site,
                        richness = Y,
                        soil_N = X1,
                        slope = X2,
...
ggpairs(data_washburn, columns = 3:7) + theme_bw()
```
...
model_washburn1 <- lm(richness ~ soil_N + slope + aspect + cover + pH, data=data_washburn)
summary (model_washburn1) #coefficients of the full model
```
\
...
-  “Added-variable” emphasizes the diagnostic use: you’re asking “what happens if I add this variable into the model?” The plot shows the unique contribution of that variable.

-  “Partial regression” emphasizes the statistical perspective: you’re looking at the regression of *residual(Y)* on *residual(X)*, which is algebraically equivalent to the partial regression coefficient.
...
```{r added variable plots, message=FALSE, warning=FALSE, fig.height=10}
car::avPlots(model_washburn1) #avPlots gives AVPs for many model types including GLMs
#olsrr::ols_plot_added_variable(model_washburn1) #only gives AVPs for linear models
```

We can also diagnose multicollinearity using the **Variance Inflation Factor (VIF)**. This measures how much the variance of a regression coefficient is “inflated” because of multicollinearity (correlations among predictors).

```{r check model for multicolinearity}
performance::check_collinearity(model_washburn1)
#VIF are also translated to Tolerance values, where tolerance = 1/vif
```
```{r check model, fig.width=8, fig.height=8}
performance::check_model(model_washburn1)
```
...
**VIF = 1**
- No correlation with other predictors; the predictor is completely independent.

**VIF between 1 and ~5**
- Some correlation with other predictors, but generally not severe. Most researchers consider this acceptable.

**VIF between ~5 and 10**
- Indicates moderate to high multicollinearity. Coefficients may be unstable and harder to interpret. You should be cautious and investigate further (e.g., check correlations, consider variable reduction or combining predictors).

**VIF > 10**
- Commonly considered a red flag for serious multicollinearity. The predictor is highly redundant with others, and coefficient estimates are likely unreliable (James et al. 2013).

The higher the VIF, the more that predictor’s estimated variance is inflated due to multicollinearity with other predictors. This doesn’t necessarily invalidate your model, but it makes interpreting individual coefficients harder and may reduce statistical power.
...
For demonstration purposes will assume that we have *apriori* justification for including all predictor variables in our model; typically we should cite 1-2 papers supporting the role of each predictor in previous studies. Remember that data dredging is strongly discouraged and can result in spurious (and irrelevant or worse, wrong) results and inference. Our model including all predictor variables was already fit above `model_washburn1`. Use `dredge` function to fit all combinations.
```{r fit dredge model}
# change na. action
options(na.action = "na.fail") # otherwise blows up with NA values
dredge_washburn <- dredge(model_washburn1)
dredge_washburn
```
...
subset(dredge_washburn, delta <5)
```
```{r equally-competitve models}
#grab equally competitive models
subset(dredge_washburn, delta <2)
```
...
sw(dredge_washburn) #notice this is the global model, not just the competitive model set
```
...
model.avg(dredge_washburn, revised.var = TRUE)
```
...
#summary(model.avg(dredge_washburn)) # if you want to average across all models, both competitive and non-competitive
summary(model.avg(dredge_washburn, subset = delta < 2)) # if you just want to look only at competitive models, which
...
```{r I guess we should make a figure, fig.width=10}
w1 <- ggplot(data_washburn, aes(soil_N, richness)) + 
  geom_point() +
  geom_smooth(method="lm")
  #scale_x_continuous(limits = c(0, 700))

w2 <- ggplot(data_washburn, aes(slope, richness)) + 
  geom_point() +
  geom_smooth(method="lm")
