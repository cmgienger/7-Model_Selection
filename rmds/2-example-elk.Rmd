...
library(ggeffects)
#loops to make plot for every predictor variable

# Multiple regression of 'top model' (same as above)
top_model <- lm(formula = density ~ distance + elev, data = data_elk)

# All variable names in the model frame
vars <- names(model.frame(top_model))[-1]  # drop the response "y"

# Make a list of ggpredict results
preds <- lapply(vars, function(v) ggpredict(top_model, terms = v))

# Helper function: build equation text
make_eqn <- function(var, coefs) {
  b0 <- round(coefs[1], 3)
  b1 <- round(coefs[var], 3)
  paste0("y = ", b0, " + ", b1, " × ", var)
}

# Convert each to a plot with title
plots <- lapply(seq_along(preds), function(i) {
  plot(preds[[i]]) + ggtitle(paste("Effect of", vars[i]))
})
...
Each of these `ggpredict` plots shows:
-  The fitted regression line (partial effect of the predictor, controlling for others)
...
```{r}
#easystats
estimate_means(top_model, c("distance")) |> plot()
```
