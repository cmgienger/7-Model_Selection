...
The data includes five predictor variables: elev (elevation), slope (gradient), area, distance (to nearest population), and pct.cover (% cover). Let's see what they look like.

```{r message=FALSE, warning=FALSE}

print(dfSummary(data_elk, 
                varnumbers   = FALSE, 
                valid.col    = FALSE, 
                graph.magnif = 0.75),
      max.tbl.height = 400, # if this is turned off, entire table displays
      method = "render")
```
...
```{r check for co-linearity, fig.height=8, fig.width=8}
...
```{r Royalls rule for strength of evidence}
...
```{r select models 95% cumulative weight criteria}
...
```{r coerce the object out.put into a data frame}
...
Let's clean this up a bit and export as a gt table and .csv file.

```{r general clean up and table formatting}

# Round selected numeric columns to 2 decimals
sel.table$logLik <- round(sel.table$logLik, 2)
sel.table$AICc   <- round(sel.table$AICc, 2)
sel.table$delta  <- round(sel.table$delta, 2)
sel.table$weight <- round(sel.table$weight, 2)

# Assume you have models already built: mod1, mod2, mod3, mod4
mods <- list(mod1, mod4, mod3, mod2) #note the order is from best to worst model

# Extract formulas (single line per model)
labs <- vapply(mods, function(m) {
  paste(deparse(formula(m), width.cutoff = 500), collapse = "")
}, character(1))

# Add model formulas as a new column
out.df <- cbind(Model = labs, as.data.frame(sel.table))
rownames(out.df) <- NULL   # cleaner row labels

# Make a gt table
gt_tbl <- gt::gt(out.df) |>
  tab_header(
    title = "Model Selection Results for Elk Habitat Selection",
    subtitle = "Formulas shown for each candidate model"
  ) |>
  fmt_number(
  ...
  cols_label(
    Model = "Formula",
    df = "K", #by convention we use K for AIC comparison 
    logLik = "LogLik",
    AICc = "AICc",
    delta = "ΔAICc",
    weight = "Weight"
...
# Save as PNG into "../tables/"
gtsave(gt_tbl, filename = "../tables/My model selection gt.png")
```
...
write.csv(out.df,"../tables/My model selection table.csv", row.names = F)
```
...
p1 <- ggplot(data_elk, aes(distance, density)) +
  ggtitle("raw data plot - distance") +
  geom_point() +
  geom_smooth(method = "lm") +
  scale_x_continuous(limits = c(0, 700) +
  scale_y_continuous(limits = c(0, 0.06)))

p2 <- ggplot(data_elk, aes(elev, density)) +
  ggtitle("raw data plot - elev") +
  geom_point() +
  geom_smooth(method = "lm") +
...
-   Note that this example probably needs a different error structure because prediction line drops to negative for distance values below 125. Lots of zeros suggests a poisson or zero-inflated poisson model might be a better fit. We will ignore this for now.


**Create partial regression plots (effect of each predictor adjusting for others)**

You can do this by plotting predicted values from the model across a grid of each predictor **while holding the others constant at their mean**. These are different than the raw data plots above.

```{r alternative partial regression plots with rug ticks at bottom}
#sjPlot might be easier
...
p4 <-
  sjPlot::plot_model(top_model, type = "pred", terms = "elev") +
  geom_rug(
    data = data_elk,
    aes(x = elev),
    inherit.aes = FALSE,
...
Each of these `sjPlot` plots shows:

-  The fitted regression line (partial effect of the predictor, after controlling for other variables [holding the others constant at their mean])
-  A 95% confidence band around the prediction
-  'rug plot' tick marks along bottom indication location of data points on the x-axis
-  Because its compatible with `ggplot2`, you could also put raw data points on with `geom_point`.

```{r include=FALSE}
#easystats
# estimate_means(top_model, c("elev")) %>% plot() +
#   geom_rug(
#     data = data_elk,
#     aes(x = elev),
#     inherit.aes = FALSE,
...
Wow, these predicted plots look similar to the raw data plots. Let's put them next to each other for comparison.

```{r combine plots for comparison, fig.width=8, fig.height=8}
(p1 + p2) / (p3 + p4)
```

If you squint just right you should be able to see that the lines for both raw variable regression plots (top plots) and the predicted variable plots from the top model (bottom plots) are the same (their axis scaling is just different). 

Why do my predicted plots and plots of raw data look the same, including regression lines? (see next lesson)
