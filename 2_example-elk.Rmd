...
## Comparing Multiple Linear Models
...
```
We can now use the `model.sel` function to conduct model selection. The default model selection criteria is Akaike’s information criteria (AIC) with small sample bias adjustment, AICc. Here we’ll create an object `out.put` that contains all of the model selection information.
```{r}
...
sel.table
```
Now that’s more like it. Models are from best to worst fitting. You can usually drop AICc if the Log Likelihood (logLik) is in the table, but we’ll leave it here. This is about ready for a report, so let’s write it to a comma separated file.
...
```
There also are `MuMin` functions for calculating model selection criteria, such as AIC, AICc, BIC and Mallows Cp, an ad hoc model selection criterion, not recommended. Here lets only compare two models using AIC.
