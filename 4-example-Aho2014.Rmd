...
importance(dredge_wash) #notice this is the global model, not just the competitive model set
```
...
summary(model.avg(dredge_wash))
#summary(model.avg(dredge_wash, subset = delta < 2)) # if you just want to look at competitive models
#there is justification for just looking at the competitive models; trying to narrow things down.
```
